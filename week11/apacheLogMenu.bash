...
function suspiciousVisitors(){
	...
	local visitsPerDay=()
	for i in "${ioc[@]}"; do
		visitsPerDay+=$(cat "$logFile" | grep "$i" | cut -d " " -f 4,1 | sort)
	done

#	local visitsPerDay=$(cat "$logFile" | grep -f "IOC.txt" | cut -d " " -f 4,1 | sort)
...
do
	...
	# Display a message, if an invalid input is given
		...
		echo "Invalid Input. Please select a valid option from the meun."
	fi
done
